...
#$Username = "usrreleasemanagement"
#$Domain= "LWO"
#$Password ="Rele4se@2015"

$Username = "allan.simoes"
$Domain= "LWO"
$Password ="1CPYvPGbEwq%"
#Param(
...
if (!$returnValue) {
    ...
    Write-verbose "Impersonate-User failed a call to LogonUser with error code: $errCode" -Verbose
...
#Successful token stored in $tokenHandle
    ...
    if (!$Quiet) {
        Write-verbose "You are now impersonating user $([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)" -Verbose
        Write-verbose "It is very important that you call Remove-ImpersonateUser when finished to revert back to your user." -Verbose
